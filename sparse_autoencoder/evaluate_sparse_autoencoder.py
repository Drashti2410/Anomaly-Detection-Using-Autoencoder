"""
Evaluate the sparse autoencoder anomaly detector.

Reconstruction errors are computed on the normal calibration split and the
anomaly reference split, then each calibrated percentile threshold is applied
to obtain precision, recall, F1, confusion-matrix counts, and per-category
recall. Results mirror the baseline evaluation so both models are directly
comparable.
"""
...
THRESHOLD_PATH = ARTIFACTS_DIR / "thresholds" / "sparse_threshold.json"
EVALUATION_DIR = ARTIFACTS_DIR / "evaluation"
METRICS_CSV_PATH = EVALUATION_DIR / "sparse_metrics.csv"
METRICS_JSON_PATH = EVALUATION_DIR / "sparse_metrics.json"
ERRORS_PATH = EVALUATION_DIR / "sparse_reconstruction_errors.npz"
...
def reconstruction_errors(
    ...
    """
    For each sample, compute its mean squared reconstruction error.

    The sparse model's forward() returns (reconstruction, activations), only
    the reconstruction is needed here.
    """
...
def category_recalls(
    anomaly_categories: np.ndarray,
    anomaly_predictions: np.ndarray,
) -> dict[str, float]:
    """
    Compute recall for each attack category.
    """
    recalls = {}
...
def evaluate_thresholds(
    ...
    """
    Apply each threshold and compute its metrics.
    """
    results = []
    ...
    for row in threshold_rows:
        ...
        result["category_recall"] = category_recalls(
            anomaly_categories,
            anomaly_predictions,
        )
    ...
    with METRICS_CSV_PATH.open("w", newline="", encoding="utf-8") as file:
        ...
        for result in results:
            ...
            for category in category_names:
                row[f"recall_{category}"] = result["category_recall"].get(
                    category, 0.0
                )
            writer.writerow(row)
...
def save_errors(
    ...
    """
    Save reconstruction errors for later visualization.
    """
    EVALUATION_DIR.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        ERRORS_PATH,
...
def evaluate() -> None:
    ...
    model = SparseAutoencoder(input_dim=checkpoint["input_dim"]).to(device)
    ...
    # allow_pickle=True is needed because anomaly_categories is an object array
    ...
    primary_key = threshold_info["primary_threshold_key"]
    primary_result = next(
        result for result in results if result["threshold_key"] == primary_key
    )
    output_summary = {
