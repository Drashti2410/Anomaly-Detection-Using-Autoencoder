"""
Calibrate anomaly thresholds for the sparse autoencoder.

The threshold is derived from reconstruction errors on the normal calibration
split (data the model was not trained on). This mirrors the baseline
calibration so the two models can be compared using the same procedure.
"""
...
def reconstruction_errors(
    ...
    """
    For each sample, compute the mean squared error between the original input
...
def calibrate_threshold() -> None:
    ...
    # load normal calibration data
    x_calibration = np.load(DATA_PATH, allow_pickle=False)[
        "X_calibration_normal"
    ].astype(np.float32)
    ...
    # use p95 as the main threshold for the sparse result
