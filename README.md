...
The project uses the **KDD Cup 1999** dataset. The original UCI KDD Cup 1999 dataset website
(`http://kdd.ics.uci.edu/databases/kddcup99/kddcup99.html`) was not accessible,
so this project uses the same KDD Cup 1999 dataset hosted on Kaggle:

- Kaggle dataset: [https://www.kaggle.com/datasets/galaxyh/kdd-cup-1999-data](https://www.kaggle.com/datasets/galaxyh/kdd-cup-1999-data)

Download the dataset from Kaggle, then place the required files in the `dataset/`
folder using **exactly** these filenames (the preprocessing notebook expects these names):


| File in `dataset/`                 | Role                            |
| ---------------------------------- | ------------------------------- |
| `kddcup.data_10_percent_corrected` | Training source (10% subset)    |
| `corrected`                        | Official labeled final test set |
| `kddcup.names.txt`                 | Feature schema / column names   |
| `training_attack_types.txt`        | Attack → category mapping       |


You can download it either manually from the Kaggle page above (click
**Download**, then unzip), or with the Kaggle CLI:

```bash
# Requires: pip install kaggle  and a Kaggle API token (~/.kaggle/kaggle.json)
kaggle datasets download -d galaxyh/kdd-cup-1999-data -p dataset --unzip
```

After downloading, make sure the four files above exist in `dataset/` with the
exact filenames. If any file is compressed (`.gz`) or uses a different name,
decompress and rename it accordingly, for example:

```bash
cd dataset
# decompress if you downloaded the .gz variants
gunzip -k kddcup.data_10_percent.gz   # -> kddcup.data_10_percent
gunzip -k corrected.gz                # -> corrected
# rename to the names expected by the preprocessing notebook
...
`Loss = MSE(reconstruction) + β · Σ KL(ρ ‖ ρ^_j)`, with sparsity target **ρ = 0.1** and sparsity weight **β = 1e-3**.
...
[https://github.com/hongyanchang/SEP740-DeepLearning](https://github.com/hongyanchang/SEP740-DeepLearning)